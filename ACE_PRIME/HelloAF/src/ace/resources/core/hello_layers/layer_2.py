...
class Layer2(Layer):
    ...
    def process_layer_messages(
        ...
        layer2_system_message = system_message.render(ace_context=ace_context, identity=identity)
        
        outputs_dir = get_outputs_dir()
        outputs_env = Environment(loader=FileSystemLoader(outputs_dir))
        l2_north = outputs_env.get_template("l2_north.md").render()
        l2_south = outputs_env.get_template("l2_south.md").render()
        ...
        layer2_instructions = layer_instructions.render(
            ...
            northbound_instructions=l2_north,
            southbound_instructions=l2_south,
        )

        llm_messages: [GptMessage] = [
            {"role": "system", "content": layer2_system_message},
